...
/**
 * @dev Check if path includes name of a zebra test file
 * @param path file path to check
 */

function isZebraTestFile(path: string) {
    return ['tests.sol', 'zebra_tests.sol', 'zebra_accounts.sol'].some(name => path.includes(name))
}
...
function processFile(filePath: string, sources: SrcIfc, isRoot: boolean = false) {
    ...
    // Return if file is a zebra test file or already processed
    if(isZebraTestFile(filePath) || isFileAlreadyInSources)
        return

    let content: string = fs.readFileSync(filePath, { encoding: 'utf-8' });
    const testFileImportRegEx: RegExp = /^(import)\s['"](zebra_tests.sol|tests.sol)['"];/gm

    // import 'zebra_tests.sol', if file is a root test contract file and doesn't already have it 
    if (isRoot && filePath.endsWith('_test.sol') && regexIndexOf(content, testFileImportRegEx) < 0) {
        const includeTestLibs: string = '\nimport \'zebra_tests.sol\';\n'
 ...
 * @param cb Callback
 * 
 * TODO: replace this with zebra's own compiler code
 */
...
export function compileFileOrFiles(filename: string, isDirectory: boolean, opts: any, cb: Function) {
    ...
    const sources: SrcIfc = {
        ...
        'zebra_tests.sol': { content: require('../sol/tests.sol.js') },
        'zebra_accounts.sol': { content: writeTestAccountsContract(accounts) }
    }
...
export function compileContractSources(sources: SrcIfc, compilerConfig: CompilerConfiguration, importFileCb: any, opts: any, cb: Function) {
    ...
    if (!('zebra_tests.sol' in sources) && !('tests.sol' in sources)) {
        sources['tests.sol'] = { content: require('../sol/tests.sol.js') }
        sources['zebra_tests.sol'] = { content: require('../sol/tests.sol.js') }
        sources['zebra_accounts.sol'] = { content: writeTestAccountsContract(accounts) }
    }
    const testFileImportRegEx: RegExp = /^(import)\s['"](zebra_tests.sol|tests.sol)['"];/gm
